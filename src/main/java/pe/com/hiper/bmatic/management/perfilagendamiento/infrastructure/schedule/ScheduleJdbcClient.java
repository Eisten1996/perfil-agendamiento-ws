...
public class ScheduleJdbcClient {
    ...
    public void saveSchedules(List<Schedule> scheduleList) {
        ...
        this.jdbcTemplate.batchUpdate(queryInsert.toString(), new BatchPreparedStatementSetter() {
    ...
    public void deleteSchedulesById(Integer schedulingId, boolean filter) {
        ...
        if (filter) {
            query = query + " AND nfecha >= CONVERT(date, GETDATE())";
        }
    ...
    public List<Schedule> getSchedulesById(Integer schedulingId) {
        ...
        return jdbcTemplate.query(query,
                (rs, rowNum) -> Schedule.builder().id(rs.getInt("NCODHORARIO"))
                        .start(rs.getString("NFECHA") + " " + rs.getString("CHORAINICIO"))
                        .end(rs.getString("NFECHA") + " " + rs.getString("CHORAFIN")).day(rs.getInt("NDIA"))
                        .schedulingId(rs.getInt("NCODPERFILAGENDAMIENTO")).counterId(rs.getString("CCODVENTANILLA"))
                        .date(rs.getString("NFECHA")).addDating(rs.getInt("CCITAADICIONAL"))
                        .bookingType(rs.getString("BOOKINGTYPE")).counterTypeId(rs.getString("COUNTERTYPEID")).build(),
